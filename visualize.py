...
#     X_min = min(top_left_x_s)
#     X_max = max(top_left_x_s)
#     Y_min = min(top_left_y_s)
#     Y_max = max(top_left_y_s)
#     arr = np.full((X_max-X_min+2*PATCH_SIZE, Y_max-Y_min+2*PATCH_SIZE, 3), 255, dtype=np.uint8)
#     for i, img_file in enumerate(img_files):
#         tl_x, tl_y = top_left_x_s[i]-X_min+PATCH_SIZE, top_left_y_s[i]-Y_min+PATCH_SIZE
#         br_x, br_y = tl_x+PATCH_SIZE, tl_y+PATCH_SIZE
#         img = cv2.imread(img_file)
#         img = cv2.resize(img, dsize=(PATCH_SIZE, PATCH_SIZE), interpolation=cv2.INTER_LINEAR)
#         arr[tl_x:br_x, tl_y:br_y] = img
#     slide_img = cv2.resize(arr, dsize=(1024,768), interpolation=cv2.INTER_LINEAR)
...
def recreate_slide(img_dir, jpg_path):
    ...
    top_left_x_s = [int(re.search("_X_\d+", f).group(0).split('_')[-1]) for f in img_files]
    top_left_y_s = [int(re.search("_Y_\d+", f).group(0).split('_')[-1]) for f in img_files]
    arr = np.full((80000, 50000, 3), 255, dtype=np.uint8)
    for i, img_file in enumerate(img_files):
